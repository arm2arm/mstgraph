...
std::vector<T> smooth(std::vector<T> &v)
{
  ...
  for(int i=2;i<np-2;i++)
    vsm[i]=(v[i-2]+2*v[i-1]+3*v[i]+2*v[i+1]+v[i+2])/9.0;
  vsm[0]=(vsm[0]+vsm[1]+vsm[2])/3.0;
  vsm[1]=(vsm[1]+vsm[2]+vsm[3])/3.0;
  vsm[np-2]=(vsm[np-2]+vsm[np-3]+vsm[np-4])/3.0;
  vsm[np-1]=(vsm[np-1]+vsm[np-2]+vsm[np-3])/3.0;
  return vsm;
}
...
std::vector< std::vector<T> > GetAB(vector<T> &R,vector<T> &x,vector<T> &y, T profRMAX=20.0, T xStep=0.1)
{
  ...
  int iR20 = std::count_if(R.begin(), R.end(),
    ...
    for (fm = 2; fm < 9; fm += 2) {
        ...
        std::fill(al0.begin(), al0.end(), 0.0);
        std::fill(al2.begin(), al2.end(), 0.0);
        std::fill(bl2.begin(), bl2.end(), 0.0);
