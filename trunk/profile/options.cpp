...
COptions::COptions(int argc, char* argv[]):m_status(0)
	...
	try {
		...
		commands.add_options()
			...
			("out-sigma-file", po::value< string>(&m_file_sigma)->default_value(string("sigma")), "Sigma file mask.")
			("update", po::value< bool>(&m_updatelog)->default_value(0), " update out file 0/1= No/Yes, if =0 then it will overwrite  out file")
