...
//////////////////////////////////////////////////////////		
	...
	MyFloat psum=0.0;
	MyFloat aq[]={0.001,0.025,0.05,0.5,0.6,0.7};//PDF quantile in the fraction
	...
	/* here we would like to test histogram */
	WList.clear();
	WList.push_back(-7);
	/* ACHTUNG!!!! */

	vector<MyFloat> taqvec( aq, &aq[ aqsize] );
	string f1=base+"Whist.txt";
	string f2=base+"WhistX.txt";
	string f3=base+"WList.txt";
	cout<<"Dumping Histograms to file...";
	DumpVector(f1, WHist, WHist.size());
	cout<<".";std::cout.flush();
	DumpVector(f2, WHistX, WHistX.size());
	cout<<".";std::cout.flush();
	DumpVector2(f3, WList, taqvec,taqvec.size());
	cout<<"DONE"<<endl;
	...
	cout<<"Start to cut MST by given Thr in W.";std::cout.flush();
	...
	for(uint iq=0;iq<WList.size();iq++)
		{
			...
			Remover<weight_map_type>	r(get(edge_weight, graphMST), LL_LENGHT*1.1);
			...
			for(int comp=0;comp<num ;comp++)
				...
				/*if(MSTCat[comp].Ntotal >MIN_NGRP)
				{
				cout<<MSTCat[comp];
				cout<<">>> Report comp="<<comp<<"  "<<MSTCat[comp].Ntotal<<endl;
				}
				else
				{
				//	cout<<"erasing comp="<<comp<<"  "<<MSTCat[comp].Ntotal<<endl;
				MSTCat[comp].Ntotal=0;
				//	MSTCat[comp].id.clear();
				// MSTCat.erase(MSTCat.begin()+comp);
...
int main()
	...
	realdata.resize(extents[data.size()][3]);
	for (i=0; i<N; i++) {
		dist=CoordDist(data[i], zero);
		realdata[i][0]=Ap[0];
		realdata[i][1]=Ap[1];
		realdata[i][2]=(dist*100);
	...
	for(i=0;i<N;i++)
		...
		iq=i;		
		qv.clear();		
		qv.push_back(realdata[iq][0]);
		qv.push_back(realdata[iq][1]);
		qv.push_back(realdata[iq][2]);
		//if(data[iq].w > meanW)
			{
			tree->n_nearest( qv , MAXNGB,ngblist);		
				{	
	...
	while(meanW <maxW)
		...
		num = connected_components(graphMST, &component[0]);
		cout<<"Number of components:"<<num<<endl;
		cout<<"..Done..It="<<It<<" Thr="<<RhoThr<<endl;
		It++;
