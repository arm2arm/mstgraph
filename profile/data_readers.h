...
class CGadget : public  CReaders
	...
	public:
		...
		template <class T> unsigned int read_block(T *&pV, const char *name, int t)
			{
			int nall;
			...
			if(t==6)
				...
				/*static const boost::regex rcw("H([0-5])");
				if( !(boost::regex_match(std::string(name), rcw)))//strcmp(name,"ID  ")
					nall=myhead.npart[0];*/
					}
				pV=new T[nall];
